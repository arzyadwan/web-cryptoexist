    ...
    <article>
      ...
      <header className="mb-8">
        ...
        <div className="flex flex-wrap items-center justify-between border-y border-gray-100 py-4 gap-4">
          ...
          <div className="flex items-center gap-3">
             <BookmarkButton articleId={post._id} title={post.title} slug={post.slug.current} />
      ...
      <div className="mt-12 bg-gray-50 border border-gray-200 rounded-xl p-6 md:p-8 flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
         ...
         <div>
            <h4 className="font-black text-gray-900 text-lg mb-1">About {post.author?.name}</h4>
            <p className="text-gray-600 text-sm leading-relaxed mb-3">
               Jurnalis teknologi yang berfokus pada perkembangan blockchain, regulasi crypto, dan Web3 di Asia Tenggara.
               {/* Note: Nanti bisa ambil dari post.author.bio jika di Sanity sudah ada field bio */}
            </p>
            <Link href={`/author/${post.author?.slug?.current}`} className="text-xs font-bold text-blue-600 hover:text-blue-800 uppercase tracking-wide">
      ...
      <div className="mt-12 p-8 bg-yellow-400 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center">
         <h3 className="text-2xl font-black text-black mb-2">Jangan Ketinggalan Berita!</h3>
         <p className="text-black/80 font-medium mb-6">Dapatkan ringkasan berita crypto harian langsung di inbox Anda.</p>
         <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" placeholder="Email Anda" className="flex-grow px-4 py-3 rounded border-2 border-black font-bold focus:outline-none" />
            <button className="bg-black text-white px-6 py-3 rounded font-black uppercase tracking-wider hover:bg-gray-800 transition">Subscribe</button>
         </div>
      </div>
      ...
      {relatedPosts.length > 0 && (
         <div className="mt-16 pt-12 border-t border-gray-200">
            <div className="flex items-center gap-3 mb-8">
               ...
               <h3 className="text-xl font-black text-gray-900 uppercase tracking-tight">Baca Juga</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               {relatedPosts.map((relatedPost: Post) => (
                  <NewsCard key={relatedPost._id} post={relatedPost} minimal={true} />
               ))}
            </div>
         </div>
      )}
