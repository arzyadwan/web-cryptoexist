...
export default async function Home() {
  const { heroPost, topNewsPosts, latestNewsPosts, trending, editorsPick, tags } = await getData()
        ...
        <section>
          {heroPost ? (
            ...
            <div className="bg-gray-100 h-64 flex items-center justify-center rounded-xl border-2 border-dashed border-gray-300">
               <p className="text-gray-400 font-bold">⚠️ Editor belum memilih Hero Banner</p>
            </div>
          )}
        </section>
                        ...
                        <div className="relative h-48 w-full bg-gray-100 overflow-hidden">
                           ...
                           <div className="absolute top-3 left-3">
                              {post.categories?.slice(0, 1).map((cat: any) => (
                                <span key={cat.slug.current} className="bg-yellow-400 text-black text-[10px] font-black px-2 py-1 rounded uppercase tracking-wide shadow-sm">
                                  {cat.title}
                                </span>
                              ))}
                           </div>
